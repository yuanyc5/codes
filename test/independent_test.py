...
if __name__ == "__main__":
    ...
    seq_file = './independent.fasta'#测试数据
    ...
    #加载测试数据
    Label_Test, KNFP, Phy12, Phy90, Embedding_Train, embedding_matrix ,phy4,phy64 = load_Train_Data(seq_file, seq_model)
    

    y_pre = model.predict({'phy4_input': phy4, 'phy12_input': Phy12, 'phy90_input': Phy90, 'embedding_input': Embedding_Train,'phy64_input':phy64})
    ypred = transfer_label_from_prob(y_pre[:, 1])
    ...
    #标签对比 
    tp,fn,tn,fp = calculate_performance(len(ytrue), ypred, ytrue)
    print("the value of tp and fn is:")
    print(tp)
    print(fn)
    print(tn)
    print(fp)
